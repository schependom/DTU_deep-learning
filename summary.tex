...
We predict the \textbf{distribution} (parameters $\bm{\theta}$) of the labels $\bm{y}$ given the inputs $\bm{x}$ using the model $f_{\bm{\phi}}(\bm{x})$:
...
\begin{itemize}
    \item Regression: $$p(\bm{y} | f_{\bm{\phi}}(\bm{x})) = p(\bm{y} | \mu_1, \ldots, \mu_k, \sigma_1, \ldots, \sigma_k) = \boxed{\mathcal{N}(\bm{y} | \mu_1, \ldots, \mu_k, \sigma_1, \ldots, \sigma_k)}$$
    \item Classification ($\bm{y} \in \{0, 1\}^K$ one-hot-encoded): $$p(\bm{y} | f_{\bm{\phi}}(\bm{x})) = p(\bm{y} | \pi_1, \ldots, \pi_K) = \boxed{\Pi_{d=1}^K \pi_d^{y_d}}$$
\end{itemize}
...
\begin{tabular}{l | l | l}
    ...
    Softmax (Output)   & $\sigma(z_d) = \pi_d = \dfrac{e^{z_d}}{\sum_d e^{z_d}}$ & Output
\end{tabular}
...
Assuming that dimensions of $\bm{y}$ are independent given $\bm{x}$:
$$p(\bm{y} | f_{\bm{\phi}}(\bm{x})) = \prod_{d=1}^D p(y_{d} | f_{\bm{\phi}}(\bm{x}))$$
which yields the loss function
$$\boxed{L(\bm{\phi}) = - \sum_{i=1}^N \sum_{d=1}^D y_{id} \log p(y_{id} | f_{\bm{\phi}}(\bm{x}_i))}$$

For multiclass classification, we had $p(\bm{y}_i | f_{\bm{\phi}}(\bm{x}_i)) = \Pi_{d=1}^K \pi_{id}^{y_{id}}$, so the \textbf{cross-entropy loss} is
$$\boxed{L(\bm{\phi}) = - \sum_{i=1}^N \sum_{d=1}^K y_{id} \log \pi_{d}}$$
with class probabilities $\pi \in [0, 1]$, which sum to 1 ($\sum_d \pi_d = 1$): $$\pi = \begin{bmatrix}
        \pi_1  \\
        \vdots \\
        \pi_K
...
TODO: chain rule derivatives in backward pass.
...
$$\sigma^2 = \frac{2}{D_{\text{in}}} \qquad \Longleftarrow \text{Var}\big[h_i^{(l)}\big] = \text{Var}\big[h_i^{(l-1)}\big]$$
...
Encoder $f_{\phi_e}: \bm{x} \to \bm{z}$, Decoder $g_{\phi_d}: \bm{z} \to \hat{\bm{x}}$.
...
\begin{itemize}
    \item \textbf{Encoder}: $f : \bm{x} \mapsto \bm{z}$
    \item \textbf{Decoder}: $g : \bm{z} \mapsto \hat{\bm{x}}$
    \item \textbf{Reconstruction}: $\hat{\bm{x}} = g(f(\bm{x}))$
    \item We want $\hat{\bm{x}} \approx \bm{x}$
\end{itemize}

\subsection{Autoencoders (AE)}

Focus on \textbf{bottleneck} autoencoders, i.e. $D_z < D_x$.

\subsubsection{Limitation}

We want to sample the latent space $\bm{z}$ using the decoder to generate new data $\hat{\bm{x}}$. \\
But we didn't make any assumptions about the distribution of $\bm{z}$, so we can't sample from it.
